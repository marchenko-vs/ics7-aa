        ...
        for (std::size_t i = 0; i < rows; i++)
        {
            ...
            for (std::size_t j = 0; j < rows_and_columns / 2; j++)
                mul_horizontal[i] = mul_horizontal[i] +
                                    matrix_1.elements[i][2 * j] * matrix_1.elements[i][2 * j + 1];
        }
        for (std::size_t i = 0; i < columns; i++)
        {
            ...
            for (std::size_t j = 0; j < rows_and_columns / 2; j++)
                mul_vertical[i] = mul_vertical[i] +
                                  matrix_2.elements[2 * j][i] * matrix_2.elements[2 * j + 1][i];
        }
        for (std::size_t i = 0; i < rows; i++)
        {
            std::vector<int> row;
            for (std::size_t j = 0; j < columns; j++)
            {
                ...
                {
                    row[j] = row[j] + (matrix_1.elements[i][2 * k] + matrix_2.elements[2 * k + 1][j]) *
                                          (matrix_1.elements[i][2 * k + 1] + matrix_2.elements[2 * k][j]);
                }
            }
            matrix_res.elements.push_back(row);
        }
    ...
    int optimized_winograd_mul(const matrix_t &matrix_1, const matrix_t &matrix_2, matrix_t &matrix_res)
    {
        ...
        for (std::size_t i = 0; i < rows; i++)
        {
            ...
            for (std::size_t j = 0; j < d; j++)
            {
                ...
                mul_horizontal[i] += matrix_1.elements[i][index] * matrix_1.elements[i][index + 1];
            }
        }
        for (std::size_t i = 0; i < columns; i++)
        {
            ...
            {
                std::size_t index = j << 1;
                mul_vertical[i] += matrix_2.elements[index][i] * matrix_2.elements[index + 1][i];
            }
        }
        for (std::size_t i = 0; i < rows; i++)
        {
            ...
            for (std::size_t j = 0; j < columns; j++)
            {
                ...
                for (std::size_t k = 0; k < d; k++)
                {
                    ...
                    row[j] += (matrix_1.elements[i][index] + matrix_2.elements[index + 1][j]) *
                              (matrix_1.elements[i][index + 1] + matrix_2.elements[index][j]);
                }
            }
            matrix_res.elements.push_back(row);
        }
        if (rows_and_columns % 2 != 0)
        {
            for (std::size_t i = 0; i < rows; i++)
            {
                for (std::size_t j = 0; j < columns; j++)
                {
                    ...
                    matrix_res.elements[i][j] += matrix_1.elements[i][index] *
