...
START_TEST(test_one_element)
{
    ...
    optimized_winograd_mul(matrix_1, matrix_2, matrix_res);
    ck_assert_int_eq(matrix_res.elements[0][0], -45);
}
END_TEST

START_TEST(test_square_matrix_even)
{
    matrix_t matrix_1, matrix_2, matrix_res;
    matrix_1.rows = matrix_1.columns = matrix_2.rows = matrix_2.columns = 4;
    int k = 1;
    ...
    int correct[4][4] = {{90, 100, 110, 120},
                         {202, 228, 254, 280},
                         {314, 356, 398, 440},
                         {426, 484, 542, 600}};
    optimized_winograd_mul(matrix_1, matrix_2, matrix_res);
...
START_TEST(test_square_matrix_odd)
{
    matrix_t matrix_1, matrix_2, matrix_res;
...
{
    Suite *s = suite_create("test optimized Winograd algorithm");
    TCase *tc = tcase_create("all tests");
    tcase_add_test(tc, test_one_element);
    tcase_add_test(tc, test_square_matrix_odd);
    tcase_add_test(tc, test_square_matrix_even);
